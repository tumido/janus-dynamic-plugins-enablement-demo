...
npx @backstage/cli@latest new --scope @$NPM_ACCOUNT --select plugin --option id=demo
...
```bash
cp -r $DEMO_DIR/src/DemoEntityCard ./src/components
...
cp $DEMO_DIR/src/app-config.yaml .
cp $DEMO_DIR/src/demo/config.d.ts .
...
yarn export-dynamic
yarn publish
npm info @$NPM_ACCOUNT/plugin-demo
```
